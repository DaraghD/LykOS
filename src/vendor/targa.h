 ...
 * Parse TGA format into pixels. Returns NULL or error, otherwise the returned
 * data looks like ret[0] = width of the image ret[1] = height of the image
 *   ret[2..] = 32 bit ARGB pixels (blue channel in the least significant byte,
 * alpha channel in the most)
 */
static unsigned int *tga_parse(unsigned char *ptr, int size) {
  serial_writeln("PARSING TGA");
  unsigned int *data;
