...
void fat16_init(void) {
  ...
  for (u32 i = 0; i < bpb.fat_size_sectors; i++)
  ...
  for (u32 i = 0; i < root_sectors; i++) {
    ...
    for (u32 j = 0; j < bpb.bytes_per_sector / 32; j++) {
      u32 entry_index = i * (bpb.bytes_per_sector / 32) + j;
...
void fat16_read_file(fat16_dir_entry_t *entry, u8 *buffer) {
  ...
  u32 bytes_left = entry->file_size;
  u8 sector_buffer[512];

  while (cluster < 0xFFF8 && bytes_left > 0) {
    u32 lba = fat16_cluster_to_lba(cluster);
    ...
    for (u32 i = 0; i < bpb.sectors_per_cluster && bytes_left > 0; i++) {
      ...
      u32 copy_size = (bytes_left > 512) ? 512 : bytes_left;
    ...
    cluster = *((u16 *)(fat_table + cluster * 2));
  }
}
...
fat16_dir_entry_t *fat16_find_file(const char *name) {
  ...
  for (u32 i = 0; i < bpb.root_entries; i++) {
...
u32 fat16_get_entries_at_entry(fat16_dir_entry_t location,
                               fat16_dir_entry_t *files) {
...
// }
//
// u8 fat16_close_file(const char *file_path) {
//   // create file on disk at file_path
//   return 1;
// }
